...
# Перетворення CSV на JSON з дозаписом нових даних
# Зчитування даних із CSV-файлу
try:
    with open('data.csv', 'r', encoding='utf-8') as csvfile:
        reader = csv.DictReader(csvfile)
        data = list(reader)  # Зчитуємо всі рядки 
# Якщо файл не знайдено
except FileNotFoundError:
    print(f"Файл '{'data.csv'}' не знайдено.")
    data = []
...
# Нові дані 
new_data = [
    {"порода": "йоркшерський терьєр", "ім'я": "Фантик", "вік (міс)": 4},
    {"порода": "ретривер", "ім'я": "Хатіко", "вік (міс)": 9}
]
...
try:
    with open('data.json', 'w', encoding='utf-8') as jsonfile:
            ...
            print("Дані успішно записані у JSON файл.")
except IOError as e:
        print("Помилка запису в JSON файл:", e)